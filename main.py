...
async def download_video(url: str, output_path: Path) -> bool:
    ...
    # Attempt 1: best quality with browser headers
    ...
    # Attempt 2: simpler format, no sections (some platforms dont support --download-sections)
    simple_cmd = [
        sys.executable, "-m", "yt_dlp",
        "--no-playlist",
...
def upload_to_gemini(video_path: Path) -> Optional[object]:
    ...
    try:
        ...
        print(f"Upload complete, state: {file.state.name}")
        # Wait for processing
        max_wait = 120
        waited = 0
        while file.state.name == "PROCESSING" and waited < max_wait:
            ...
            print(f"File state after {waited}s: {file.state.name}")
        if file.state.name == "FAILED":
            print("Gemini file processing FAILED")
            return None
        print(f"File ready: {file.state.name}")
        return file
    except Exception as e:
...
def analyze_with_gemini(gemini_file) -> Optional[dict]:
    ...
    try:
        ...
        response = model.generate_content(
            [gemini_file, "Analyze this video forensically and return your assessment as JSON."],
            generation_config={
                "temperature": 0.1,
